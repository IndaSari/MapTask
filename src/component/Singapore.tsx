...
const Singapore = () => {
  ...
  const [bbox, setBbox] = useState({ lat: 1.285194, lng: 103.8522982 });

  const [taxiList, setTaxiList] = useState([]);


  const getTaxiList = async () => {
    const response = await fetch('/api/drivers?latitude=1.285194&longitude=103.8522982',
  ...
  const taxiSlider = async (value: any) => {
    const response = await fetch('/api/drivers?latitude=1.285194&longitude=103.8522982&count=' + value);
    if (response.status === 200) {
      const content = await response.json();
