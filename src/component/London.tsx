...
const London = () => {
  ...
  const [bbox, setBbox] = useState({ lat: 51.5049375, lng: -0.0964509 });

  const [taxiList, setTaxiList] = useState([]);


  const getTaxiList = async () => {
    const response = await fetch('/api/drivers?latitude=51.5049375&longitude=-0.0964509',
  ...
  const taxiSlider = async (value: any) => {
    const response = await fetch('/api/drivers?latitude=51.5049375&longitude=-0.0964509&count=' + value);
    if (response.status === 200) {
      const content = await response.json();
      ...
      >
        ...
        <LocationMarker bbox={bbox} setBbox={setBbox} />
        {
          taxiList.map((key: any) => (
            <TaxiMarker mark={{ lat: key.location.latitude.toString(), lng: key.location.longitude.toString() }} />
          ))
        }
      </MapContainer>
    </div>
